...
class HeatStrokePredictor(object):
	...
	def make_prediction(self, user_attributes, heart_rate_stream):
		...
		# Heat
		temp = self.user_attributes['Environmental temperature (C)']
		humidity = self.user_attributes['Relative Humidity']
		sun = self.user_attributes['Exposure to sun']
		HI_prob = self.make_heat_index_prediction(humidity, temp, sun=sun)
		
		# Core Temp Estimation
		CT_prob = self.core_temperature_risk
